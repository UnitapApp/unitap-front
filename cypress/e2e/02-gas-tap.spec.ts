...
import { ChainType, ClaimReceipt, ClaimReceiptState } from "../../types";
...
const setupGetChainListServerGeneral = () => {
  ...
  cy.intercept(
    {
      method: "POST",
      url: "/api/gastap/chain/**/claim-max",
    },
    (req) => {
      const chain = chainList.find((item) => item.pk === 2)!;

      collectedChains.push(
        createClaimHistory(chain as any, ClaimReceiptState.VERIFIED, chain.pk!)
      );
...
describe("can claim gasses", () => {
  ...
  it("should show the claim cards empty", () => {
    ...
    cy.get("[data-testid=claims-chain-list]")
      .should("be.visible")
      .and("not.be.empty");
  });
  ...
  it("filters chain types correctly", () => {
    ...
    chainList.forEach((chain) => {
      if (chain.chainType === ChainType.NONEVM)
        cy.get(`[data-testid=chain-name-${chain.pk}]`).should("exist");
    });
  });
