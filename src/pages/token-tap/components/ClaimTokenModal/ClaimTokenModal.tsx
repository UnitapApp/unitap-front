...
const ClaimTokenModalBody = ({chain}: { chain: Chain }) => {
	...
	const {selectedTokenForClaim, handleClaimToken, claimTokenLoading} = useContext(TokenTapContext);
				...
				>
					{
						claimTokenLoading ?
							<p>Claiming...</p>
							:
							<p>{`Claim ${formatWeiBalance(selectedTokenForClaim.chain.maxClaimAmount)} ${selectedTokenForClaim.chain.symbol}`}</p>
					}
				</ClaimButton>
			</>
		);
...
const ClaimTokenModal = () => {
	...
	return (
		<Modal
			title={`Claim ${formatWeiBalance(selectedTokenForClaim.chain.maxClaimAmount)} ${selectedTokenForClaim.chain.symbol}`}
			size="small"
			closeModalHandler={closeClaimTokenModal} isOpen={isOpen}>
