          ...
          <section className={"flex--1"}>
            <Link className={"flex--1"} to={RoutePath.TOKEN}>
              ...
              >
                {sortedChainList.length > 0 && (
                  <div
                    className={"flex flex-col text-xs text-white bg-gray30 rounded-xl py-3 px-3 items-start justify-between mb-2"}
                  >
                  <span className="token-logo-container w-6 h-6 mb-3">
                    <img src={sortedChainList[0].logoUrl} alt={sortedChainList[0].chainName}
                         className="token-logo w-auto h-[100%]" />
                  </span>
                    <p className="text-xs mb-2">{sortedChainList[0].chainName}</p>
                    <p className="text-xs text-gray100 leading-6">
                      The first 2000 users who are verified in Aura can claim 2 xDai.
                    </p>
                  </div>
                )}
              </Widget>
