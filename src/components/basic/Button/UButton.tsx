...
const UButton = (props: UButtonProps) => {
	const { className, buttonClassName, children, size, onClick, icon, unClickable, disabled } = props;
	...
	return (
		<div
			...
			className={`before:rounded-lg rounded-lg ${size ? size : 'btn-default'} ${className ? className : 'text-sm'} ${
				...
				unClickable || disabled ? 'cursor-default' : 'cursor-pointer before:cursor-pointer'
