"use client";

import { useEffect, useState } from "react";
import DatePicker from "react-multi-date-picker";
import TimePicker from "react-multi-date-picker/plugins/time_picker";
import "react-multi-date-picker/styles/layouts/mobile.css";
import { EnrollmentDurationsProps, ErrorProps } from "@/types";

interface EndDateCompProp {
  showErrors?: ErrorProps | null;
  handleSetEnrollDuration: (id: number) => void;
  enrollmentDurations: EnrollmentDurationsProps[];
  updateEndDateTime: number | null
  setUpdatedEndTime: (time: number) => void
  currentEndTime: number
}

const EndDateComp = ({ handleSetEnrollDuration, enrollmentDurations, updateEndDateTime, setUpdatedEndTime, currentEndTime }: EndDateCompProp) => {

  const [endDate, setEndDate] = useState<any>();
  const [minDate, setMinDate] = useState<any>();

  useEffect(() => {
    setMinDate(Date.now());
  }, []);

  const changeTime = (e: any) => {
    if (e?.unix) {
      setUpdatedEndTime(new Date(e.unix * 1000).setSeconds(0) / 1000)
    }
    setEndDate(e);
  };

  useEffect(() => {
    const checkList = enrollmentDurations.find((item) => item.selected);
    if (checkList) {
      setEndDate(null);
    }
  }, [enrollmentDurations]);

  useEffect(() => {
    let newEndTimeStamp: any;
    if (currentEndTime) {
      let selectedDuration: EnrollmentDurationsProps =
        enrollmentDurations.filter((item) => item.selected == true)[0];
      if (!selectedDuration) return;
      if (selectedDuration.status == "week") {
        newEndTimeStamp =
          currentEndTime + selectedDuration.value * 7 * 24 * 60 * 60;
      }

      if (selectedDuration.status == "month") {
        const currentDate = new Date(currentEndTime * 1000);

        newEndTimeStamp = Math.round(
          currentDate.setMonth(
            Number(currentDate.getMonth()) + selectedDuration.value,
          ) / 1000,
        );
      }
    }
    if (newEndTimeStamp) {
      setUpdatedEndTime(newEndTimeStamp)
    }
  }, [enrollmentDurations, currentEndTime]);

  const timePickerClick = (openCalendar: any) => {
    openCalendar();
    handleSetEnrollDuration(-1);
  };

  return (
    <div className="relative">
      <div className={`flex`}>
        <DatePicker
          // disabled={!data.startTimeStamp}
          style={{
            border: "none",
            background: "none",
            color: "#b5b5c6",
            display: "none",
          }}
          containerStyle={{
          }}
          name="endTime"
          format="DD/MM/YYYY - hh:mm A"
          plugins={[<TimePicker key={0} position="bottom" hideSeconds />]}
          render={(value, openCalendar) => {
            return (
              <p
                className="select-none mt-2 text-xs"
                onClick={() => timePickerClick(openCalendar)}
              >
                or Select Date & Time Manually
              </p>
            );
          }}
          onChange={changeTime}
          value={endDate}
          minDate={minDate}
          className="rmdp-mobile animate-fadeIn"
        />
      </div>
      <p className="absolute m-0 -mt-[.5px] p-0 text-[11px] text-error ">
        {/* {showErrors && !data.endTimeStamp && showErrors.endDateStatusMessage} */}
      </p>
    </div>
  );
};

export default EndDateComp;
