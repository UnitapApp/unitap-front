...
export default function PrizeCard({ prize, isHighlighted }: PrizeCardProps) {
  return (
    ...
    >
      <div className="bg-circle-dots flex flex-wrap items-stretch gap-y-2 p-[2px] md:flex-nowrap">
        ...
        <div className="rotate-3 rounded-xl border-2 bg-[#E5FFE2]">
          ...
          <div className="absolute right-0 top-1 grid h-12 w-12 -translate-y-1/2 translate-x-1/2 place-items-center overflow-hidden rounded-full border bg-white shadow-primary-button-sm">
            <img
              width={47}
              height={47}
              src={prize.chain.logoUrl}
              alt={prize.chain.chainName}
            />
          </div>
        </div>
      </div>
...
export const PrizeDetails: FC<{ prize: Prize; isHighlighted?: boolean }> = ({
  prize,
  isHighlighted,
}) => {
  ...
  return (
    <div
      className={`relative flex w-full flex-col gap-4 overflow-hidden rounded-3xl p-5 md:w-72 ${isHighlighted ? "bg-[#000] p-5 text-white" : "bg-gray-full text-black-0"}`}
    >
      <div
        className={`absolute right-0 top-0 w-48 translate-x-1/4 translate-y-1/2 rotate-[40deg] border-b-2 border-t-2 border-black-0 bg-landing-raffle py-1 text-center font-bold text-black-0 ${isHighlighted ? "text-black-0" : ""}`}
      >
        Raffle
      </div>
      ...
      <LandingButton
        onClick={() => openEnrollModal(prize, "Winners")}
